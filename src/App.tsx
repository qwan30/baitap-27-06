...
import { User as AppUser, Meeting, DashboardStats, TranscriptSegment, MeetingSummary, ActionItem } from './types';
import { Interactive3DTour } from './components/Interactive3DTour';
...
// Specialized ReactMarkdown renderer with word-by-word streaming typing simulation and elegant styling
const TypingMarkdown: React.FC<{ text: string; isLastAssistant: boolean }> = ({ text, isLastAssistant }) => {
  ...
  useEffect(() => {
    ...
    }, 25); // high velocity natural flow formatting

    return () => clearInterval(interval);
  }, [text, isLastAssistant]);

  return (
    <div className="text-[12px] text-gray-800 leading-relaxed font-sans space-y-1 py-0.5">
      <ReactMarkdown
        components={{
          h1: ({node, ...props}) => <h1 className="text-xs font-bold text-gray-900 mt-2.5 mb-1 border-b border-gray-100 pb-0.5" {...props} />,
          h2: ({node, ...props}) => <h2 className="text-xs font-bold text-gray-800 mt-2 mb-1" {...props} />,
          h3: ({node, ...props}) => <h3 className="text-xs font-bold text-gray-750 mt-1.5 mb-0.5" {...props} />,
          ...
          strong: ({node, ...props}) => <strong className="font-bold text-gray-950 bg-amber-50 px-0.5" {...props} />,
          code: ({node, ...props}) => <code className="bg-gray-150 font-mono text-[11px] px-1.5 py-0.5 rounded border border-gray-200 text-red-600 font-medium" {...props} />,
          pre: ({node, ...props}) => <pre className="bg-gray-50 border border-gray-250 rounded-md p-2 overflow-x-auto my-1.5 text-[10.5px] font-mono leading-normal text-gray-700 shadow-inner" {...props} />,
          a: ({node, ...props}) => <a className="text-blue-600 hover:underline font-semibold" target="_blank" rel="noopener noreferrer" {...props} />,
        }}
      >
        {displayedText}
      </ReactMarkdown>
...
// Safe access helper to protect against SecurityError inside blocked client-side sandboxed iframes
const safeLocalStorage = {
  getItem: (key: string): string | null => {
    try {
      return typeof window !== 'undefined' && window.localStorage ? localStorage.getItem(key) : null;
    } catch (e) {
      console.warn('localStorage is blocked or not available in sandbox:', e);
      return null;
    }
  },
  setItem: (key: string, value: string): void => {
    try {
      ...
      console.warn('localStorage setItem blocked or not available in sandbox:', e);
    }
  },
  removeItem: (key: string): void => {
    try {
      ...
      console.warn('localStorage removeItem blocked or not available in sandbox:', e);
    }
  }
};

function base64ToBlob(base64: string, mimeType: string = 'audio/webm'): Blob {
  // Sanitize base64 by removing all whitespaces and potential headers
  const sanitized = base64.replace(/\s/g, '').replace(/^data:audio\/[^;]+;base64,/, '');
...
export default function App() {
  ...
  // App navigation
  const [activeTab, setActiveTab] = useState<'dashboard' | 'meetings' | 'new-meeting' | 'tasks' | 'templates' | 'settings'>('dashboard');
  ...
  const [showWelcomeTour, setShowWelcomeTour] = useState(() => safeLocalStorage.getItem('meetEchoTourDismissed') !== 'true');

  // Advanced Transcript and Speaker states
  ...
  const [renamingSpeakerName, setRenamingSpeakerName] = useState<string | null>(null);
  const [renamingSpeakerNewName, setRenamingSpeakerNewName] = useState<string>('');
  ...
  useEffect(() => {
    const cachedUser = safeLocalStorage.getItem('mi_user');
    if (cachedUser) {
      try {
        setCurrentUser(JSON.parse(cachedUser));
      } catch (e) {
        safeLocalStorage.removeItem('mi_user');
      }
    }
  }, []);
  ...
  // Cleanup Object URL and Audio on unmount or meeting change
  useEffect(() => {
    return () => {
  ...
  // Synchronized HTML5 audio playback and interval simulation
  useEffect(() => {
    if (audioInstanceRef.current) {
      audioInstanceRef.current.pause();
      audioInstanceRef.current = null;
  ...
  useEffect(() => {
    ...
    if (isAudioPlaying && selectedMeeting) {
      const isDummyAudio = selectedMeeting.audioBase64 && 
        (selectedMeeting.audioBase64.length < 500 || 
         selectedMeeting.audioBase64 === "UklGRigAAABXQVZFZm10IBAAAAABAAEARKwAAIhYAQACABAAZGF0YQQAAAAAAA==");

      if (selectedMeeting.audioBase64 && !isDummyAudio) {
        let rawSrc = '';
        try {
          const base64DataOnly = selectedMeeting.audioBase64.startsWith('data:')
            ? selectedMeeting.audioBase64.split(',')[1]
            : selectedMeeting.audioBase64;
          const mime = selectedMeeting.audioBase64.startsWith('data:')
            ? selectedMeeting.audioBase64.split(';')[0].split(':')[1]
            : 'audio/webm';
          
          const blob = base64ToBlob(base64DataOnly, mime);
          const objUrl = URL.createObjectURL(blob);
          rawSrc = objUrl;
          objectUrlRef.current = objUrl;
        } catch (e) {
          console.warn('Error creating object URL, using raw base64 data string as fallback:', e);
          rawSrc = selectedMeeting.audioBase64.startsWith('data:') 
            ? selectedMeeting.audioBase64 
            : `data:audio/webm;base64,${selectedMeeting.audioBase64}`;
        }

        if (!audioInstanceRef.current || audioInstanceRef.current.src !== rawSrc) {
          audioInstanceRef.current = new Audio(rawSrc);
          audioInstanceRef.current.playbackRate = playbackSpeed;
          
          audioInstanceRef.current.addEventListener('timeupdate', () => {
            if (audioInstanceRef.current && selectedMeeting.durationSec) {
              const progress = (audioInstanceRef.current.currentTime / selectedMeeting.durationSec) * 100;
              setPlaybackProgress(Math.min(100, Math.round(progress)));
            }
          });
        ...
        try {
          audioInstanceRef.current.play().catch(err => {
            console.warn("Real audio playback blocked or failed, falling back to simulated ticks:", err);
            fallbackToSimulation = true;
          });
        } catch (playErr) {
          console.warn("Synchronous audio playback failed, falling back to simulated ticks:", playErr);
          fallbackToSimulation = true;
        }
      } else {
        fallbackToSimulation = true;
      ...
      playInterval = setInterval(() => {
        setPlaybackProgress(prev => {
          ...
          // If we are playing real audio, rely on timeupdate instead of simulated clock ticks
          if (selectedMeeting.audioBase64 && audioInstanceRef.current && !fallbackToSimulation) {
            return prev;
          }
          
          const steps = selectedMeeting.durationSec || 120;
          const increment = (1 / steps) * 100;
  ...
  }, [isAudioPlaying, selectedMeeting?.id]);
  ...
  const handleSelectMeeting = async (meet: Meeting) => {
    // 1. Render lightweight metadata immediately so frontend renders detailing UI with zero lag
    setSelectedMeeting(meet);
    setActiveTab('meetings');
    
    // 2. Load the actual massive meeting payload (including audioBase64 string stream) asynchronously
    try {
      const response = await fetch(`/api/v1/meetings/${meet.id}`);
      if (response.ok) {
        const fullMeeting: Meeting = await response.json();
        // Safeguard to only set if user is still actively focused on this particular meeting
        setSelectedMeeting(prev => prev && prev.id === meet.id ? fullMeeting : prev);
      }
    } catch (err) {
      console.error('Error retrieving full meeting details binary stream:', err);
    }
  };
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCurrentUser(user);
      safeLocalStorage.setItem('mi_user', JSON.stringify(user));
  ...
  const startRecordingFlow = async () => {
    ...
    try {
      ...
      // Double checks native model support checklist for safe iOS Safari compatibility
      ...
      } else if (typeof MediaRecorder.isTypeSupported === 'function' && MediaRecorder.isTypeSupported('audio/ogg')) {
  ...
  const stopAndProcessRecording = async () => {
    ...
    try {
      ...
      let effectiveSimulationCategory = simulationCategory;

      const hasCapturedChunks = audioChunksRef.current && audioChunksRef.current.length > 0;

      if (simulationCategory === 'real-mic' && mediaRecorder && hasCapturedChunks) {
        let audioBlob: Blob;
        ...
        if (simulationCategory === 'real-mic') {
          effectiveSimulationCategory = targetMeeting.language === 'Vietnamese' ? 'mock-vietnamese' : 'mock-design';
          setSimulationCategory(effectiveSimulationCategory);
        }
        setProcessingStatus('Connecting pre-recorded benchmark sample metrics to Gemini...');
        b64Data = COMPACT_VALID_WAV_BASE64;
        finalDuration = effectiveSimulationCategory === 'mock-vietnamese' ? 88 : 142;
      }
      ...
      const processResponse = await fetch(`/api/v1/meetings/${targetMeeting.id}/process`, {
        ...
        body: JSON.stringify({
          ...
          simulationCategory: effectiveSimulationCategory,
  ...
  const toggleActionItemStatus = async (meetingId: string, actionId: string, currentStatus: 'pending' | 'completed') => {
    const nextStatus: 'pending' | 'completed' = currentStatus === 'pending' ? 'completed' : 'pending';
    
    // Optimistic UI updates
    setMeetings(prev => prev.map(m => {
      if (m.id === meetingId && m.actionItems) {
        const nextActions: ActionItem[] = m.actionItems.map(act => act.id === actionId ? { ...act, status: nextStatus } : act);
    ...
    if (selectedMeeting && selectedMeeting.id === meetingId && selectedMeeting.actionItems) {
      const updatedActions: ActionItem[] = selectedMeeting.actionItems.map(act => act.id === actionId ? { ...act, status: nextStatus } : act);
  ...
  const triggerExportDownload = async (meetingId: string, format: 'markdown' | 'txt') => {
    try {
      ...
      a.href = url;
      a.download = `meeting_${meetingId}_export.${format === 'markdown' ? 'md' : 'txt'}`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      showToast(`Meeting exported successfully as ${format === 'markdown' ? 'Markdown' : 'Text'}.`);
    } catch (err) {
      console.warn("Direct blob download failed, falling back to window.open", err);
      try {
        window.open(`/api/v1/meetings/${meetingId}/export?format=${format}`, '_blank');
      } catch (e2) {
        showToast('Export failed due to browser restriction.');
      }
    }
  };

  const handleUpdateSegmentText = async (segIdx: number, newText: string) => {
    if (!selectedMeeting || !selectedMeeting.transcript) return;
    
    const updatedTranscript = selectedMeeting.transcript.map((seg, i) => 
      i === segIdx ? { ...seg, text: newText } : seg
    );

    const updatedMeeting = { ...selectedMeeting, transcript: updatedTranscript };
    ...
    try {
      const response = await fetch(`/api/v1/meetings/${selectedMeeting.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ transcript: updatedTranscript })
      });
      if (!response.ok) throw new Error();
      showToast('Transcript segment updated successfully.');
    } catch (err) {
      showToast('Error syncing transcript edit to server.');
    }
  };

  const handleRenameSpeaker = async (oldLabel: string, newLabel: string) => {
    if (!selectedMeeting || !selectedMeeting.transcript || !newLabel.trim()) return;

    const updatedTranscript = selectedMeeting.transcript.map(seg => 
      seg.speakerLabel === oldLabel ? { ...seg, speakerLabel: newLabel } : seg
    );

    const updatedActionItems = selectedMeeting.actionItems ? selectedMeeting.actionItems.map(act => 
      act.assignee === oldLabel ? { ...act, assignee: newLabel } : act
    ) : [];
    ...
    try {
      ...
      if (!response.ok) throw new Error();
      showToast(`Speaker "${oldLabel}" renamed to "${newLabel}" globally.`);
    } catch (err) {
      showToast('Error syncing speaker rename to cloud database.');
    }
  };

  const handleAddTask = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedMeeting || !taskFormTitle.trim()) return;

    const newTask: ActionItem = {
      id: 'act_' + Math.random().toString(36).substr(2, 9),
      title: taskFormTitle,
      assignee: taskFormAssignee || 'Unassigned',
    ...
    const updatedActions = [...(selectedMeeting.actionItems || []), newTask];
    const updatedMeeting = { ...selectedMeeting, actionItems: updatedActions };
    ...
    try {
      ...
      showToast('Error saving new task to server.');
    }
  };

  const handleDeleteTask = async (taskId: string) => {
    if (!selectedMeeting || !selectedMeeting.actionItems) return;
    if (!window.confirm('Are you sure you want to delete this action task?')) return;

    const updatedActions = selectedMeeting.actionItems.filter(act => act.id !== taskId);
    const updatedMeeting = { ...selectedMeeting, actionItems: updatedActions };
    ...
    try {
      ...
      showToast('Error deleting task on server.');
    }
  };
      ...
      <header className="sticky top-0 z-40 bg-white border-b border-[#e5e7eb] px-6 h-16 flex items-center justify-between">
        <div className="flex items-center gap-6">
          <div 
            onClick={() => { 
              if (activeRecordingMeeting) {
                if (!window.confirm("You have an active recording in progress. Leaving this page will stop and discard your current session. Do you want to return to the Home page?")) {
                  return;
                }
                setActiveRecordingMeeting(null);
                setIsRecordingPaused(false);
            ...
            className="flex items-center gap-2 cursor-pointer group hover:opacity-85 transition-all p-1.5 rounded-lg hover:bg-gray-50 active:scale-[0.98]"
          ...
          <footer className="bg-[#101010] text-[#a1a1aa] py-16 px-6 border-t border-[#1a1a1a]">
            <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-5 gap-8 text-left">
              ...
              <div 
                ...
                className="space-y-4 col-span-2 md:col-span-1 cursor-pointer group"
              >
                <span className="font-bold text-white tracking-tight text-lg group-hover:text-blue-400 transition-colors">meetecho<span className="text-blue-500">.ai</span></span>
        ...
        <div className="min-h-screen flex flex-col md:flex-row bg-[#ffffff]">
          ...
          <aside className="w-full md:w-60 bg-white border-r border-[#e5e7eb] flex flex-col pt-3 shrink-0 text-left">
            ...
            <nav className="flex-1 px-2 space-y-1">
              ...
              >
                ...
                {meetings.length > 0 && (
                  <span className={`ml-auto px-1.5 py-0.5 rounded text-[10px] font-mono leading-none ${
                    activeTab === 'meetings' && !selectedMeeting ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-500 border border-gray-200'
                  }`}>
                    {meetings.length}
                  </span>
                )}
              ...
              >
                ...
                <span>Tasks</span>
                {meetings.reduce((acc, m) => acc + (m.actionItems ? m.actionItems.filter(t => t.status === 'pending').length : 0), 0) > 0 && (
                  <span className={`ml-auto px-1.5 py-0.5 rounded text-[10px] font-mono leading-none ${
                    activeTab === 'tasks' ? 'bg-white/20 text-white' : 'bg-amber-150 text-amber-750 border border-amber-200'
                  }`}>
                    {meetings.reduce((acc, m) => acc + (m.actionItems ? m.actionItems.filter(t => t.status === 'pending').length : 0), 0)}
                  </span>
                )}
              </button>
              ...
              {/* 5. Agendas & Templates */}
              <button
                onClick={() => { setActiveTab('templates'); setSelectedMeeting(null); }}
                className={`w-full flex items-center gap-3 px-4 py-2 text-xs font-semibold rounded-md transition ${
                  activeTab === 'templates'
                    ? 'bg-[#111111] text-white shadow-sm' 
                    : 'text-gray-500 hover:text-[#111111] hover:bg-[#f5f5f5]'
            ...
            <div className="p-4 border-t border-[#e5e7eb] space-y-2">
              <button
                ...
                className="w-full px-3 py-2 bg-[#f9fafb] hover:bg-[#f3f4f6] text-gray-700 border border-[#e5e7eb] rounded-md text-xs font-semibold transition flex items-center justify-center gap-1.5"
              >
                <Sparkles className="w-3.5 h-3.5 text-yellow-500 fill-yellow-400" />
                <span>Guided Visual Tour</span>
              </button>
          ...
          <main className="flex-1 flex flex-col min-w-0 bg-[#ffffff]">
            ...
            <div className="flex-1 p-6 md:p-10 space-y-8 overflow-y-auto max-w-6xl mx-auto w-full">
                        ...
                        <div className="bg-white border border-[#e5e7eb] p-5 rounded-lg space-y-3">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
                            ...
                            <button
                              type="button"
                              onClick={() => {
                                setSimulationCategory('mock-design');
                                setNewMeetingLang('English');
                              }}
                              className={`p-3 rounded-lg border text-xs font-semibold flex flex-col items-center justify-center gap-1.5 transition text-center cursor-pointer ${
                            ...
                            <button
                              type="button"
                              onClick={() => {
                                setSimulationCategory('mock-vietnamese');
                                setNewMeetingLang('Vietnamese');
                              }}
                              className={`p-3 rounded-lg border text-xs font-semibold flex flex-col items-center justify-center gap-1.5 transition text-center cursor-pointer ${
              ...
              {/* TAB ACTIVE PANEL 2: DASHBOARD OVERVIEW */}
              {activeTab === 'dashboard' && !selectedMeeting && (
                <DashboardOverview
                  meetings={meetings}
              ...
              {/* TAB ACTIVE PANEL 2.5: MEETINGS LIBRARY */}
              {activeTab === 'meetings' && !selectedMeeting && (
                <div className="space-y-6 text-left animate-fade-in">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="space-y-1">
                      <h2 className="text-2xl font-bold tracking-tight text-[#111111] font-sans">Meetings Library</h2>
                      <p className="text-xs text-gray-500">Transcribe drafts, export markdown reports, or manage existing dialogue vectors.</p>
                    </div>
                  ...
                  {/* Styled light gray text stats strip as requested */}
                  <div className="py-2.5 px-4 bg-gray-50 border border-gray-150 rounded-lg text-xs text-gray-500 font-sans flex flex-wrap items-center gap-1.5 font-semibold/90 shadow-2xs">
                    <span className="font-extrabold text-[#111111]">{meetings.length}</span>
                    <span>meetings</span>
                    <span className="text-gray-300">•</span>
                    <span className="font-extrabold text-[#111111]">{meetings.filter(m => m.status === 'completed').length}</span>
                    <span>notes ready</span>
                    <span className="text-gray-300">•</span>
                    <span className="font-extrabold text-[#111111]">
                      {meetings.reduce((acc, m) => acc + (m.actionItems ? m.actionItems.filter(t => t.status === 'pending').length : 0), 0)}
                    </span>
                    <span>open tasks pending guidance</span>
                  </div>
                      ...
                      {meetings.map((meet) => {
                        ...
                        return (
                          <motion.div
                            whileHover={{ y: -2 }}
                            key={meet.id}
                            className="bg-white border border-[#e5e7eb] p-6 rounded-lg transition-all flex flex-col justify-between space-y-4 shadow-sm group hover:border-[#111111] font-sans"
                          >
                            <div className="space-y-2">
                              ...
                              <h3 
                                ...
                                className="text-base font-semibold text-[#111111] transition-colors duration-200 cursor-pointer hover:text-blue-600 font-sans line-clamp-1 text-left"
                              >
                                {meet.title}
                              </h3>

                              {meet.description && (
                                <p className="text-xs text-gray-500 line-clamp-2 leading-relaxed font-sans text-left">{meet.description}</p>
                              )}
                            </div>
                                ...
                                {meet.status === 'completed' ? (
                                  ...
                                  meet.status === 'draft' && (
                                    <button
                                      ...
                                      className="text-blue-600 hover:bg-white rounded transition flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold uppercase font-sans"
                                    >
              ...
              {/* TAB ACTIVE PANEL 3: TASKS REGISTRY */}
              {activeTab === 'tasks' && !selectedMeeting && (
                <TasksManager
                  meetings={meetings}
                  onToggleActionItem={toggleActionItemStatus}
                  onOpenMeetingDetail={(meeting) => {
                    handleSelectMeeting(meeting);
                  }}
                />
              )}

              {/* TAB ACTIVE PANEL 4: MEETING TEMPLATES */}
              {activeTab === 'templates' && !selectedMeeting && (
                <MeetingTemplates
                  onSelectTemplate={(title, desc, type) => {
                    setNewMeetingTitle(title);
                    setNewMeetingDesc(desc);
                ...
                <div className="space-y-6 text-left">
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            ...
                            <div className="flex items-center gap-1.5 bg-gray-100 p-0.5 rounded-full border border-gray-200 self-start sm:self-center">
                              {['1.0x', '1.25x', '1.5x'].map((speedOpt) => {
                                const numericSpeed = parseFloat(speedOpt);
                                const isActive = playbackSpeed === numericSpeed;
                                return (
                                  <button
                                    key={speedOpt}
                                    onClick={() => {
                                      setPlaybackSpeed(numericSpeed);
                                      showToast(`Playback speech speed toggled to ${speedOpt}`);
                                    }}
                                    className={`px-2.5 py-1 text-[10px] font-bold font-mono rounded-full transition cursor-pointer ${isActive ? 'bg-[#111111] text-white shadow-sm' : 'text-gray-500 hover:text-[#111111] hover:bg-white/50'}`}
                                  >
                                    {speedOpt}
                                  </button>
                                );
                           ...
                           <div className="space-y-1">
                             <div className="relative w-full h-1.5 bg-gray-200 hover:bg-gray-250 rounded-full cursor-pointer group" onClick={(e) => {
                               ...
                               const totalDuration = selectedMeeting.durationSec || 120;
                               const currentSec = Math.floor((clickedProgress / 100) * totalDuration);
                               setPlaybackProgress(clickedProgress);
                               if (audioInstanceRef.current) {
                                 audioInstanceRef.current.currentTime = currentSec;
                               }
                               showToast(`Seeking dialogue time to ${currentSec}s`);
                             }}>
                            ...
                            <div className="flex items-center justify-between text-[9px] font-mono text-gray-400">
                              <span>
                                {Math.floor(((playbackProgress / 100) * (selectedMeeting.durationSec || 120)) / 60).toString().padStart(2, '0')}:
                                {Math.floor(((playbackProgress / 100) * (selectedMeeting.durationSec || 120)) % 60).toString().padStart(2, '0')}
                              </span>
                              <span>
                                {Math.floor((selectedMeeting.durationSec || 120) / 60).toString().padStart(2, '0')}:
                                {Math.floor((selectedMeeting.durationSec || 120) % 60).toString().padStart(2, '0')}
                              </span>
                            </div>
                          </div>
                        ...
                        <div className="space-y-4 max-h-[440px] overflow-y-auto pr-2">
                          {filteredTranscript.length === 0 ? (
                            ...
                            filteredTranscript.map((seg, idx) => {
                              ...
                              const originalIdx = selectedMeeting.transcript ? selectedMeeting.transcript.indexOf(seg) : -1;
                              const isEditing = editingSegmentIdx === originalIdx;
                              const isRenaming = renamingSpeakerName === seg.speakerLabel;

                              return (
                                <div 
                                  key={idx} 
                                  className="flex gap-4 items-start border-l-2 border-gray-250 hover:border-blue-500 pl-4 py-2 transition-all group hover:bg-gray-50/70 rounded-r-md"
                                >
                                  {/* Interaction Tooltip: Click to play dialogue audio */}
                                  <button
                                    onClick={() => {
                                      // calculate progress based on segment's start time and duration
                                      const duration = selectedMeeting.durationSec || 120;
                                      const progressPercent = Math.min(100, Math.round((seg.startTimeSec / duration) * 100));
                                      setPlaybackProgress(progressPercent);
                                      if (audioInstanceRef.current) {
                                        audioInstanceRef.current.currentTime = seg.startTimeSec;
                                      }
                                      setIsAudioPlaying(true);
                                      showToast(`Playing dialogue starting at ${minFmt}:${secFmt}`);
                                    }}
                                    title="Click to play dialogue audio segment"
                                    className="font-mono text-[10.5px] font-bold text-gray-700 hover:text-white bg-gray-150 group-hover:bg-blue-600 border border-gray-200 px-2 py-0.5 rounded shrink-0 select-none shadow-sm transition flex items-center gap-1 cursor-pointer"
                                  >
                                    <Play className="w-2.5 h-2.5 fill-current opacity-70 group-hover:opacity-100" />
                                    <span>{minFmt}:{secFmt}</span>
                                  </button>

                                  <div className="space-y-1.5 text-left flex-1">
                                    {/* Speaker Label rename block */}
                                    {isRenaming ? (
                                      <div className="flex items-center gap-2">
                                        <input
                                          type="text"
                                          value={renamingSpeakerNewName}
                                          onChange={(e) => setRenamingSpeakerNewName(e.target.value)}
                                          className="bg-white border-2 border-blue-400 rounded px-2 py-0.5 text-xs font-bold text-blue-900 focus:outline-none"
                                          autoFocus
                                        />
                                        <button
                                          onClick={() => handleRenameSpeaker(seg.speakerLabel, renamingSpeakerNewName)}
                                          className="text-[10px] bg-blue-600 hover:bg-blue-700 text-white px-2 py-1 rounded font-bold transition shadow-sm"
                                        >
                                          Save
                                        </button>
                                        <button
                                          onClick={() => setRenamingSpeakerName(null)}
                                          className="text-[10px] bg-gray-200 hover:bg-gray-300 text-gray-700 px-2 py-1 rounded transition"
                                        >
                                          Cancel
                                        </button>
                                      </div>
                                      ...
                                      <div className="flex items-center gap-1.5">
                                        <span className="text-[11px] text-[#111111] font-bold block bg-gray-100 px-2 py-0.5 rounded border border-gray-200 shadow-xs">
                                          👤 {seg.speakerLabel}
                                        </span>
                                        <button
                                          onClick={() => {
                                            setRenamingSpeakerName(seg.speakerLabel);
                                            setRenamingSpeakerNewName(seg.speakerLabel);
                                          }}
                                          title="Rename this speaker globally"
                                          className="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-blue-600 transition p-0.5 rounded"
                                        >
                                    ...
                                    {/* Segment text editing inline */}
                                    {isEditing ? (
                                      <div className="space-y-1.5">
                                        <textarea
                                          value={editingSegmentText}
                                          onChange={(e) => setEditingSegmentText(e.target.value)}
                                          className="w-full bg-white border-2 border-blue-400 rounded-md p-2 text-xs text-gray-800 focus:outline-none font-sans"
                                          rows={2}
                                          autoFocus
                                        />
                                        <div className="flex items-center gap-2">
                                          <button
                                            onClick={() => handleUpdateSegmentText(originalIdx, editingSegmentText)}
                                            className="text-[10.5px] bg-[#111111] hover:bg-gray-800 text-white px-3 py-1 rounded-md font-semibold transition flex items-center gap-1 shadow-sm"
                                          >
                                          ...
                                          <button
                                            ...
                                            className="text-[10.5px] bg-gray-150 hover:bg-gray-200 text-gray-700 px-3 py-1 rounded-md transition"
                                          >
                                            Discard
                                          </button>
                                        </div>
                                      ...
                                      <div className="flex justify-between items-start gap-4">
                                        <p 
                                          onDoubleClick={() => {
                                            setEditingSegmentIdx(originalIdx);
                                            setEditingSegmentText(seg.text);
                                          }}
                                          title="Double click text to edit inline"
                                          className="text-xs text-gray-700 font-sans leading-relaxed flex-1 cursor-text select-text"
                                        >
                                        ...
                                        <button
                                          ...
                                          title="Correct transcript text"
                                          className="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-blue-600 transition p-1 rounded-full bg-white border border-gray-100 shadow-sm"
                                        >
                                          <Edit3 className="w-3 h-3" />
                                        </button>
                                      </div>
                  ...
                  {activeDetailTab === 'tasks' && (
                    <motion.div 
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      className="bg-white border border-[#e5e7eb] rounded-lg p-6 max-w-3xl space-y-5 text-left"
                    >
                      <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                        <div className="space-y-0.5">
                          <span className="text-sm font-bold text-[#111111] block">Action Schedule Task Workflow</span>
                          <span className="text-[10px] text-gray-400 font-mono">Synchronized realtime task items status management</span>
                        </div>
                        <button
                          onClick={() => setShowTaskForm(!showTaskForm)}
                          className="flex items-center gap-1.5 bg-[#111111] hover:bg-gray-800 text-white px-3 py-1.5 text-xs font-semibold rounded-md transition shadow-md cursor-pointer"
                        >
                          <Plus className="w-3.5 h-3.5" />
                          <span>{showTaskForm ? 'Hide Creator' : 'Create Task'}</span>
                        </button>
                      </div>
                      ...
                      {showTaskForm && (
                        <motion.form 
                          ...
                          className="bg-gray-50 border-2 border-gray-250 p-4 rounded-lg space-y-3"
                        >
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div className="space-y-1">
                              <label className="text-[10px] uppercase font-bold text-gray-500">Task Title *</label>
                              <input
                                type="text"
                                required
                                value={taskFormTitle}
                                onChange={(e) => setTaskFormTitle(e.target.value)}
                                placeholder="Verify color contrasts..."
                                className="w-full bg-white border border-gray-300 rounded p-1.5 text-xs text-gray-800 focus:outline-none focus:border-blue-500"
                              />
                            </div>
                            <div className="space-y-1">
                              <label className="text-[10px] uppercase font-bold text-gray-500">Assignee Name</label>
                              <input
                                type="text"
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            ...
                            <div className="space-y-1">
                              <label className="text-[10px] uppercase font-bold text-gray-500">Task Priority</label>
                              <select
                          ...
                          <div className="space-y-1">
                            <label className="text-[10px] uppercase font-bold text-gray-500">Description / Subtasks</label>
                            <textarea
                              value={taskFormDesc}
                              onChange={(e) => setTaskFormDesc(e.target.value)}
                              placeholder="Describe subtasks or details..."
                              className="w-full bg-white border border-gray-300 rounded p-1.5 text-xs text-gray-800 focus:outline-none focus:border-blue-500"
                              rows={2}
                            />
                          </div>
                          ...
                          <div className="flex justify-end gap-2 pt-1 border-t border-gray-200">
                            <button
                              ...
                              className="bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold px-3 py-1.5 text-xs rounded transition"
                            >
                              Discard
                            </button>
                            <button
                              type="submit"
                              className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-4 py-1.5 text-xs rounded shadow-sm transition"
                            >
                              Add Action Task
                            </button>
                          </div>
                      ...
                      {selectedMeeting.actionItems && selectedMeeting.actionItems.length > 0 ? (
                        <div className="space-y-3 pt-1">
                          {selectedMeeting.actionItems.map((act) => {
                            ...
                            const priorityVal = act.priority || 'medium';

                            return (
                              <div 
                                key={act.id}
                                className={`p-4 bg-white hover:bg-gray-50/50 border rounded-lg flex items-start justify-between gap-4 transition shadow-sm relative group/task ${
                                  isCompleted ? 'border-gray-200 bg-gray-50/40 opacity-75' : 'border-[#e5e7eb]'
                                }`}
                              >
                                <div className="flex gap-3.5 items-start flex-1 min-w-0">
                                  ...
                                  <button 
                                    ...
                                    className={`w-5 h-5 rounded border-2 flex items-center justify-center shrink-0 mt-0.5 transition cursor-pointer select-none ${
                                      isCompleted
                                        ? 'bg-[#111111] border-[#111111] text-white'
                                  ...
                                  <div className="space-y-1 min-w-0 flex-1">
                                    <div className="flex flex-wrap items-center gap-2">
                                      <span className={`text-[12.5px] font-bold transition-all truncate leading-tight ${
                                      ...
                                      {priorityVal === 'low' && (
                                        <span className="text-[9px] bg-gray-100 text-gray-600 font-bold tracking-wider uppercase px-2 py-0.5 rounded border border-gray-200">
                                          Low Priority
                                        </span>
                                      )}
                                      {priorityVal === 'medium' && (
                                        <span className="text-[9px] bg-blue-50 text-blue-700 font-bold tracking-wider uppercase px-2 py-0.5 rounded border border-blue-100">
                                          Med Priority
                                        </span>
                                      )}
                                      {priorityVal === 'high' && (
                                        <span className="text-[9px] bg-orange-50 text-orange-700 font-bold tracking-wider uppercase px-2 py-0.5 rounded border border-orange-100">
                                          High Priority
                                        </span>
                                      )}
                                      {priorityVal === 'urgent' && (
                                        <span className="text-[9px] bg-red-50 text-red-600 font-extrabold tracking-wider uppercase px-2 py-0.5 rounded border border-red-100 animate-pulse flex items-center gap-0.5">
                                          ⚠️ Urgent priority
                                        </span>
                                      )}
                                    </div>
                                    ...
                                    {act.description && (
                                      <p className="text-[11px] text-gray-500 leading-normal font-sans font-medium pl-0.5">
                                        {act.description}
                                      </p>
                                    )}

                                    {/* Sub detail footer tags */}
                                    <div className="flex flex-wrap gap-x-3 gap-y-1 font-mono text-[9px] text-gray-400 pt-0.5">
                                      <span className="font-semibold text-gray-600">👤 Owned by: <strong className="text-gray-800">{act.assignee}</strong></span>
                                      <span>•</span>
                                      <span className="font-semibold">📅 Deadline: <strong className="text-gray-800">{act.dueDate}</strong></span>
                                    </div>
                                  </div>
                                </div>

                                {/* Delete Task control button */}
                                <button
                                  onClick={() => handleDeleteTask(act.id)}
                                  title="Delete action task"
                                  className="opacity-0 group-hover/task:opacity-100 text-gray-400 hover:text-red-600 p-1 rounded-full transition ml-2 shrink-0 bg-white hover:bg-red-50 border border-gray-150"
                                >
                        ...
                        <div className="text-center py-12 space-y-1">
                          ...
                          <p className="text-xs text-gray-500">No active action items identified. Click "Create Task" to manually append steps.</p>
                        </div>
                      )}
                    </motion.div>
                  )}
            ...
            {isAiAssistantOpen && (
              <>
                ...
                >
                  ...
                  <div className="flex-1 overflow-y-auto p-5 space-y-4 relative min-h-[250px]">
                    {aiChatHistory.length === 0 ? (
                      ...
                      aiChatHistory.map((chat, idx) => {
                        ...
                        const isLastAssistant = isAssistant && idx === aiChatHistory.length - 1;

                        return (
                          <div
                            key={idx}
                            className={`flex ${chat.sender === 'user' ? 'justify-end' : 'justify-start'} text-left group`}
                          >
                            <div className="max-w-[85%] space-y-1 relative">
                              <div
                                className={`rounded-lg px-4 py-2.5 shadow-sm transition-all ${
                                  chat.sender === 'user'
                                    ? 'bg-blue-600 text-white rounded-br-none text-xs font-sans whitespace-pre-wrap leading-relaxed'
                                    : 'bg-[#f7f7f7] text-gray-800 rounded-bl-none border border-gray-200.5 px-4 py-3'
                                }`}
                              >
                                {chat.sender === 'user' ? (
                                  chat.text
                                ) : (
                                  <TypingMarkdown text={chat.text} isLastAssistant={isLastAssistant} />
                                )}
                              </div>
                              
                              {chat.sender === 'assistant' && (
                                <div className="flex justify-end pr-1 opacity-0 group-hover:opacity-100 transition duration-150">
                                  <button
                                    type="button"
                                    onClick={() => {
                                    ...
                                    className="flex items-center gap-1 text-[9px] font-mono font-semibold text-gray-400 hover:text-blue-600 transition"
                    ...
                    {/* Majestic AI Thinking backdrop-blur loading overlay */}
                    <AnimatePresence>
                      {aiAssistantLoading && (
                        <motion.div
                          initial={{ opacity: 0 }}
                          animate={{ opacity: 1 }}
                          exit={{ opacity: 0 }}
                          className="absolute inset-0 bg-white/75 backdrop-blur-[5px] flex flex-col items-center justify-center text-center p-6 space-y-4 z-20"
                        >
                          {/* Delicate spinning gradient ring and breathing pulses */}
                          <div className="relative flex items-center justify-center">
                            <div className="absolute w-14 h-14 rounded-full bg-blue-500/10 animate-ping" />
                            <div className="absolute w-10 h-10 rounded-full bg-indigo-500/15 animate-pulse" />
                            <div className="w-10 h-10 rounded-full border-4 border-t-blue-600 border-r-indigo-500 border-b-purple-500 border-l-gray-100 animate-spin" />
                            <Sparkles className="absolute w-4 h-4 text-blue-600 animate-pulse" />
                          </div>

                          <div className="space-y-1">
                            <p className="text-xs font-bold text-gray-950 animate-pulse font-sans">MeetEcho AI is thinking...</p>
                            <p className="text-[10.5px] text-gray-500 max-w-[255px] font-medium leading-relaxed font-sans">
                              Analyzing discussion transcripts to formulate deep response context.
                            </p>
                          </div>
                        </motion.div>
                      )}
