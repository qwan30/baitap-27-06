...
app.get('/api/v1/meetings', (req, res) => {
  ...
  // Exclude heavy base64 audio data for faster lists rendering to avoid browser and network footprint crashes
  const lightweightMeetings = userMeetings.map((m: any) => {
    const { audioBase64, ...rest } = m;
    return rest;
  });

  res.json(lightweightMeetings);
});
...
app.post('/api/v1/meetings/:id/process', async (req, res) => {
  const { id } = req.params;
  const { audioBase64, durationSec, simulationCategory, mimeType } = req.body;
  ...
  try {
    ...
    const isSimulation = simulationCategory === 'mock-design' || simulationCategory === 'mock-vietnamese';
    const hasApiKey = !!process.env.GEMINI_API_KEY;

    let resData: any = {};

    if (isSimulation || !hasApiKey) {
      // Simulate/mock behavior with high-fidelity datasets immediately
      if (simulationCategory === 'mock-vietnamese' || (isVietnamese && !hasApiKey)) {
        resData = {
          transcript: [
            { startTimeSec: 0, endTimeSec: 15, speakerLabel: "Lâm", text: "Chào cả nhà, dự án ra mắt dòng sản phẩm cà phê Robusta chuẩn hạt mộc đang rất tiềm năng." },
            { startTimeSec: 16, endTimeSec: 32, speakerLabel: "Vy", text: "Hạt Robusta Lâm Đồng phơi nhà màng tự nhiên mang hương vị dâu rừng độc đáo, chắc chắn Gen Z sẽ thích!" },
            { startTimeSec: 33, endTimeSec: 50, speakerLabel: "Minh", text: "Em đề xuất hộp đựng bã mía ép thân thiện môi trường để tăng giá trị sinh thái cho dòng cà phê mộc." },
            { startTimeSec: 51, endTimeSec: 72, speakerLabel: "Lâm", text: "Tuyệt hảo! Minh tập trung làm mẫu thử bao bì bã mía nhé. Vy phụ trách chuẩn bị slide song ngữ giới thiệu nhà nông." },
            { startTimeSec: 73, endTimeSec: 88, speakerLabel: "Vy", text: "Dạ vâng anh, em sẽ gửi bản nháp slide vào thứ Sáu tuần này để cả nhóm duyệt trước khi trình bày." }
          ],
          summary: {
            shortSummary: "Quyết định sản xuất cà phê Robusta mộc Di Linh chất lượng cao phối hợp bao bì thân thiện môi trường và slide đấu thầu.",
            detailedSummary: "Buổi họp tập trung thống nhất bộ giá trị cốt lõi cho hạt Robusta chất lượng cao Di Linh. Đội ngũ thông qua kế hoạch bao gói bã mía thân thiện môi trường từ thiết kế của Minh và thúc đẩy slide đấu thầu song ngữ của Vy để hoàn tất hồ sơ.",
            keywords: ["Cà phê mộc", "Robusta chất lượng cao", "Bao bì bã mía", "Gen Z"],
            risks: ["Nguồn hộp bã mía có thể bị chậm đơn giao nhận do giãn cách logistics của nhà cung cấp."]
          },
          actionItems: [
            { id: "act_103", title: "Thiết kế mẫu túi zip cà phê từ bã mía thương hiệu", assignee: "Minh", dueDate: "2026-06-25", status: "pending" },
            { id: "act_104", title: "Hoàn thiện slide giới thiệu quy trình sản xuất song ngữ", assignee: "Vy", dueDate: "2026-06-26", status: "pending" }
          ],
          decisions: [
            { id: "dec_102", title: "Dùng 100% Robusta Nhà Màng Di Linh", description: "Tập trung truyền thông câu chuyện liên kết nông hộ trực tiếp nâng cao đời sống bà con." }
          ]
        };
      } else {
        resData = {
          transcript: [
            { startTimeSec: 0, endTimeSec: 15, speakerLabel: "Alex Johnson", text: "Welcome to our design sync! Today we are discussing the premium layout alignment." },
            { startTimeSec: 16, endTimeSec: 35, speakerLabel: "Jane Smith", text: "I suggest using glowing borders with neon cyan over dark translucent glass grids." },
            { startTimeSec: 36, endTimeSec: 55, speakerLabel: "Developer Team", "text": "Indeed! We also need an automatic beforeunload warn safeguard for live recordings." },
            { startTimeSec: 56, endTimeSec: 75, speakerLabel: "Jane Smith", text: "And the immediate download button should trigger pristine Markdown export outputs." },
            { startTimeSec: 76, endTimeSec: 98, speakerLabel: "Alex Johnson", text: "Those are excellent. Let's aim to have the complete prototype wrapped up by next Tuesday." }
          ],
          summary: {
            shortSummary: "Modern glassmorphic dark purple visual style spec selected with recording safe safeguards and instant Markdown exports.",
            detailedSummary: "We reviewed design layouts for the active recording screen. The team approved high-contrast neon styling to meet strict WCAG contrast scores. Developer Team is assigned to write a browser refresh dialog safeguard so draft voice sessions are not lost prematurely.",
            keywords: ["Design System", "Glassmorphism", "WCAG Contrast", "Markdown Export"],
            risks: ["Translucent rendering can be intensive on older mobile screens; layout must remain lightweight."]
          },
          actionItems: [
            { id: "act_101", title: "Verify neon cyan borders against WCAG contrast scores", assignee: "Jane Smith", dueDate: "2026-06-25", status: "pending" },
            { id: "act_102", title: "Implement recording browser unloading safety dialog", assignee: "Developer Team", dueDate: "2026-06-24", status: "pending" }
          ],
          decisions: [
            { id: "dec_101", title: "Translucent Grayscale UI Approved", description: "High accessibility and glowing state indicators will be standardized." }
          ]
        };
      }
    } else {
      ...
      try {
        ...
        try {
          ...
          // Let's clean it up or attempt secondary fallback if some text holds markdown characters
          const cleaned = outputText.substring(outputText.indexOf('{'), outputText.lastIndexOf('}') + 1);
          resData = JSON.parse(cleaned);
        }
      } catch (geminiError: any) {
        console.warn('Gemini generateContent in the /process route failed or was overloaded, falling back to simulated high-fidelity data. Error:', geminiError);
        
        // Populate standard notice
        db.meetings[meetingIdx].processingError = 'System is temporarily highly loaded. High-fidelity backup offline-sync engine was executed successfully.';
    ...
    // Populate record
    db.meetings[meetingIdx].audioBase64 = audioBase64;
...
app.post('/api/v1/meetings/:id/regenerate', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.warn('Gemini regenerate content failed or was overloaded, falling back to simulated high-fidelity synthesis. Error:', geminiError);
      if (isVietnamese) {
        parsed = {
          summary: {
            shortSummary: meeting.summary?.shortSummary || "Bản tóm tắt cuộc họp đồng bộ thảo luận nhóm.",
            detailedSummary: (meeting.summary?.detailedSummary || "Buổi thảo luận thiết kế đồng bộ các giải pháp.") + " (Đã tối ưu hóa ngoại tuyến)",
            keywords: meeting.summary?.keywords || ["Họp nhóm", "Đồng bộ"],
            risks: meeting.summary?.risks || []
          },
          actionItems: meeting.actionItems || [
            { id: "act_new_1", title: "Cập nhật tài liệu kỹ thuật dự án", assignee: "Lâm", dueDate: "2026-06-25", status: "pending" }
          ],
          decisions: meeting.decisions || [
            { id: "dec_new_1", title: "Tiếp tục cải tiến giao diện", description: "Duy trì màu nền tối và độ tương phản cao." }
          ]
        };
      } else {
        parsed = {
          summary: {
            shortSummary: meeting.summary?.shortSummary || "A cohesive summary of the synchronized meeting progress.",
            detailedSummary: (meeting.summary?.detailedSummary || "Detailed sync discussion covering key action paths and structural adjustments.") + " (Locally synthesized fallback)",
            keywords: meeting.summary?.keywords || ["Team Sync", "Progress"],
            risks: meeting.summary?.risks || []
          },
          actionItems: meeting.actionItems || [
            { id: "act_new_1", title: "Refine technical design blueprints", assignee: "Alex Johnson", dueDate: "2026-06-25", status: "pending" }
          ],
          decisions: meeting.decisions || [
            { id: "dec_new_1", title: "Interface enhancements approved", description: "Retain high-contrast premium theme as standardized." }
          ]
        };
      }
    }
    ...
    console.error('Critical failure in manual regenerate:', error);
    res.json(db.meetings[meetingIdx]);
  }
});
...
app.post('/api/v1/meetings/:id/ask', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.warn('Gemini generateContent in the /ask route failed, generating answers with local template engine. Error:', geminiError);
      
      const qLower = question.toLowerCase();
      if (qLower.includes('decision') || qLower.includes('quyết định')) {
        if (meeting.decisions && meeting.decisions.length > 0) {
          answerText = isVietnamese 
            ? `Dưới đây là các quyết định chính trong cuộc họp:\n` + meeting.decisions.map(d => `- **${d.title}**: ${d.description}`).join('\n')
            : `Here are the key decisions from the meeting:\n` + meeting.decisions.map(d => `- **${d.title}**: ${d.description}`).join('\n');
        } else {
          answerText = isVietnamese ? 'Không có quyết định chính nào được ghi nhận.' : 'No key decisions were recorded.';
        }
      } else if (qLower.includes('action') || qLower.includes('task') || qLower.includes('việc') || qLower.includes('nhiệm vụ')) {
        if (meeting.actionItems && meeting.actionItems.length > 0) {
          answerText = isVietnamese
            ? `Các công việc cần thực hiện:\n` + meeting.actionItems.map(a => `- **${a.title}** (Người làm: ${a.assignee}, Hạn chót: ${a.dueDate})`).join('\n')
            : `Assigned action items for the team:\n` + meeting.actionItems.map(a => `- **${a.title}** (Assignee: ${a.assignee}, Due: ${a.dueDate})`).join('\n');
        } else {
          answerText = isVietnamese ? 'Không có công việc nào được phân công.' : 'No action items were assigned.';
        }
      } else {
        answerText = isVietnamese
          ? `[Phản hồi ngoại tuyến] Thảo luận hiện tại: "${meeting.title}". Cuộc họp ghi nhận các thông tin cốt lõi:\n` +
            `- Mô tả: ${meeting.description || 'N/A'}\n` +
            `- Tóm tắt: ${meeting.summary ? meeting.summary.shortSummary : 'N/A'}\n\n` +
            `Do hệ thống Gemini đang tải cao, tôi đã soạn phản hồi cơ bản này dựa trên dữ liệu cuộc họp đã lưu.`
          : `[Offline Response System] Currently asking about: "${meeting.title}". Core insights recovered:\n` +
            `- Description: ${meeting.description || 'N/A'}\n` +
            `- Summary: ${meeting.summary ? meeting.summary.shortSummary : 'N/A'}\n\n` +
            `Because Gemini services are currently under heavy load, I have synthesized this local response for your convenience.`;
      }
    }

    res.json({ answer: answerText || 'Sorry, I couldn\'t formulate an answer.' });
  } catch (error: any) {
